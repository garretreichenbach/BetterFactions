...
/**
 * HorizontalButtonPane.java
 * <Description>
 * ==================================================
 * Created 02/06/2021
 * @author TheDerpGamer
 */
public class HorizontalButtonPane extends GUIInnerTextbox {

    private GUIHorizontalButton[] buttons;
    private final int spacing;

    public HorizontalButtonPane(InputState inputState, int spacing) {
        super(inputState);
    ...
    public GUIHorizontalButton addButton(String text, GUIHorizontalArea.HButtonType type, GUICallback callback) {
        ...
        button.setWidth(getContent().getWidth() / (float) buttons.length - (float) (spacing * 2));
        button.setPos((float) spacing + (button.getWidth() + (float) spacing) * (float) buttons.length - 1, (float) spacing, 0.0F);
    ...
    public GUIExpandableButton addExpandableButton(String text, GUIHorizontalArea.HButtonType type, float expandedHeight) {
        expandSize();
        float buttonWidth = getContent().getWidth() / (float) buttons.length - (float) (spacing * 2);
        final GUIScrollablePanel expandedPanel = new GUIScrollablePanel(buttonWidth, expandedHeight, getState());
