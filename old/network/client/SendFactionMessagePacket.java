...
public class SendFactionMessagePacket extends Packet {
    ...
    public void processPacketOnServer(PlayerState playerState) {
        ...
        if(org.schema.game.common.data.player.faction.FactionManager.isNPCFactionOrPirateOrTrader(toId)) (new FactionMessage(to, from, "Reply from " + to.getName(), FactionMessageUtils.getResponseMessage(FactionMessage.MessageType.valueOf(messageType), to, from), FactionMessage.MessageType.REPLY)).sendMessage();
        else (new FactionMessage(from, to, title, message, FactionMessage.MessageType.valueOf(messageType))).sendMessage();
    }
}